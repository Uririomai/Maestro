...
func (i *Admin) SignIn(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	...
	if errors.Is(err, io.EOF) {
		logger.Error(ctx, "[SignIn] request body is empty")
		render.Status(r, http.StatusBadRequest)
		render.JSON(w, r, model.NewErrResp("empty request"))
		return
	}
	if err != nil {
		logger.Error(ctx, "[SignIn] failed to decode request body", "err", err)
		render.Status(r, http.StatusBadRequest)
	...
	if errors.Is(err, model.ErrInvalidEmail) {
		logger.Error(ctx, "[SignIn] email is invalid", "email", req.Email)
		render.Status(r, http.StatusBadRequest)
		render.JSON(w, r, model.NewErrResp("email is invalid"))
		return
	}
	if errors.Is(err, model.ErrWrongEmailOrPassword) {
		logger.Error(ctx, "[SignIn] wrong email or password", "email", req.Email)
		render.Status(r, http.StatusBadRequest)
		render.JSON(w, r, model.NewErrResp("wrong email or password"))
		return
	}
	if err != nil {
		logger.Error(ctx, "[SignIn] failed to sign in", "err", err)
	...
	if err != nil {
		logger.Error(ctx, "[SignIn] failed to generate token", "err", err)
		render.Status(r, http.StatusInternalServerError)
		render.JSON(w, r, model.NewErrResp("failed to generate token"))
		return
	}

	logger.Info(ctx, "[SignIn] admin signed in", "email", req.Email)
